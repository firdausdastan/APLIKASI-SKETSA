...
export const TextOverlay: React.FC<TextOverlayProps> = ({
  ...
  const handleMouseMove = (e: MouseEvent) => {
    ...
    // Scale delta if needed based on zoom (not implemented here but kept in mind)
    ...
    <div className="absolute inset-0 pointer-events-none overflow-hidden">
      {elements.map(el => (
        ...
        >
          {el.text}
          {selectedId === el.id && (
             <div className="absolute -top-3 -right-3 w-6 h-6 bg-white border border-primary rounded-full" />
          )}
        </div>
      ))}
    </div>
  ...
  return (
    <div className="space-y-4">
       <div className="flex items-center justify-between">
         ...
         <button onClick={() => {
             // We can't delete directly from here without parent callback, 
             // but assume opacity 0 is 'deleted' for now or handle in parent
